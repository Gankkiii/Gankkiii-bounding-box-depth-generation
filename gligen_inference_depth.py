...
def prepare_batch(meta, batch=1, max_objs=30):
    ...
    depth_info = torch.zeros(max_objs)
    ...
    for idx, (box, depth, text_feature, image_feature) in enumerate(zip( meta['locations'], meta['depth'], text_features, image_features)):
        boxes[idx] = torch.tensor(box)
        masks[idx] = 1
        depth_info[idx] = torch.tensor(depth)
    ...
    out = {
        ...
        "depth_info" : depth_info.unsqueeze(0).repeat(batch,1),
...
if __name__ == "__main__":
    ...
    meta_list = [ 
        ...
        dict(
            ckpt = "./gligen_checkpoints/checkpoint_V2.2.pth",
            prompt="chair",
            # phrases = ['a teddy bear', 'a bird'],
            phrases=['chair'],
            #locations=[[0.48828125, 0.484375, 0.86328125, 0.80859375], [0.5625, 0.10546875, 0.90625, 0.6484375], [0.0625, 0.140625, 0.48046875, 0.609375], [0.2734375, 0.73046875, 0.81640625, 0.99609375]],
            locations=[[0.0, 0.09, 0.33, 0.76]],
            # depth = [0,0,0,200],
            depth=[2500],
    ...
    starting_noise = None
    run(meta_list[0],args,starting_noise)

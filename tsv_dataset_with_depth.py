...
class TSVDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        #annos_depth_values = [anno['depth_info'] for anno in annos]

        #if len(annos) > 0:
        #    max_depth = max(annos_depth_values)
        #    min_depth = min(annos_depth_values)
        ...
        for anno in annos:
            ...
            if valid:
                ...
                all_depth.append( torch.tensor(anno['depth_info']))
                all_masks.append(1)
        ...
        depth = torch.zeros(self.max_boxes_per_data)
